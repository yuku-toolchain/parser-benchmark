...
fn unzipUcd(gpa: std.mem.Allocator, path: []const u8) !void {
    const dir_path = std.fs.path.dirname(path) orelse
        std.debug.panic("invalid ucd dict path.", .{});

    const extract_dir_path = try std.fs.path.join(gpa, &.{ dir_path, "ucd" });
    defer gpa.free(extract_dir_path);
    ...
    defer fzip.close();
    var buffer: [1024]u8 = undefined;
    var fzip_reader = fzip.reader(&buffer);

    try std.zip.extract(dir, &fzip_reader, .{});
...
fn parseFile(gpa: std.mem.Allocator, contents: []const u8) !struct { U32Set, U32Set } {
    var id_starts = U32Set.init(gpa);
    var id_contts = U32Set.init(gpa);
...
fn toTrie(gpa: std.mem.Allocator, set: *const U32Set) !struct { []u32, []u64 } {
    ...
    var chunk_at_index_root: std.AutoArrayHashMapUnmanaged(usize, Chunk) = .empty;
    defer chunk_at_index_root.deinit(gpa);

    // maps a 512-bit chunk to its offset in the array of flat chunks
    // (a.k.a. leaf node of trie).
    var leaf_offset_of_chunk: std.AutoArrayHashMapUnmanaged(Chunk, usize) = .empty;
    defer leaf_offset_of_chunk.deinit(gpa);
    ...
    for (0..n_chunks) |i_chunk| {
        ...
        try chunk_at_index_root.put(gpa, i_chunk, chunk);
        const gop = try leaf_offset_of_chunk.getOrPut(gpa, chunk);
    ...
    var root = try gpa.alloc(u32, n_chunks);
    for (0..n_chunks) |i| {
    ...
    var leaf: std.ArrayList(u64) = .empty;
    defer leaf.deinit(gpa);

    for (leaf_offset_of_chunk.keys()) |*pieces| {
        for (pieces) |x| {
            try leaf.append(gpa, x);
        }
    }

    return .{ root, try leaf.toOwnedSlice(gpa) };
}
...
fn writeTrie(
    ...
    const root, const leaf = try toTrie(gpa, codepoint_set);
    defer gpa.free(root);
    defer gpa.free(leaf);
...
fn writeFile(gpa: std.mem.Allocator, id_starts: *const U32Set, id_contt: *const U32Set) !void {
    const cwd = std.fs.cwd();
    const file = try cwd.createFile(out_file, .{});
    defer file.close();
    ...
    try writeTrie(gpa, id_starts, writer, "is_id_start");
    try writeTrie(gpa, id_contt, writer, "is_id_continue");
...
fn readAndParseFile(gpa: std.mem.Allocator, filepath: []const u8) !struct { U32Set, U32Set } {
    ...
    return try parseFile(gpa, contents);
}
...
pub fn downloadAndParseProperties(gpa: std.mem.Allocator) !struct { U32Set, U32Set } {
    if (!exists(extracted_path)) {
        const zip_path = "/tmp/ucd.zip";
        try downloadUnicodeSpec(gpa, zip_path);
        try unzipUcd(gpa, zip_path);
    }

    const derived_properties_filepath = try std.fs.path.join(gpa, &.{
        extracted_path,
        "DerivedCoreProperties.txt",
    });

    defer gpa.free(derived_properties_filepath);
    return try readAndParseFile(gpa, derived_properties_filepath);
}
...
pub fn main() !void {
    ...
    var id_starts, var id_contts =
        try downloadAndParseProperties(gpa);

    defer id_starts.deinit();
    defer id_contts.deinit();

    try writeFile(gpa, &id_starts, &id_contts);
}
